...
model_means_cld %>%
  ...
  kbl(escape = F) %>% 
  kable_paper("hover", position = "left", full_width = FALSE)
...
As you can see, 
  ...
  1. We set up a model using `lm()`
     * This is a very simple example. You may use much more complex models and [many other model classes](https://cran.r-project.org/web/packages/emmeans/vignettes/models.html).
  2. [`emmeans()`](https://www.rdocumentation.org/packages/emmeans/versions/1.6.2-1/topics/emmeans) estimates means per group
...
```{r, fig.width=5, fig.height=4, class.source = 'fold-hide'}
pwpp(model_means$emmeans) + theme_bw()
```
