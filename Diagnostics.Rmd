...
Take this as a very simplified example, starting with a dataset with 5 datapoints for variables `x` and `y`.
...
ggplot(data=dat, aes(x=x, y=y)) +
  ...
  geom_point(size = 4) +
  geom_point(aes(y = y_hat), shape = 1, size = 2, color = "blue") +
  ylim(0,NA) + xlim(0,NA) +
  stat_smooth(method = "lm", formula = 'y ~ x', se = FALSE) +
  theme_bw()
```
