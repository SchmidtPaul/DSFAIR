...
As can be seen, the F-test finds the `G:N` interaction effects to be statistically significant (p < 0.05). As a result, we should compare the adjusted means for all genotype-nitrogen level combinations.
...
all_mean_comparisons <- mod %>%
  emmeans(pairwise ~ N:G,
          adjust = "tukey",
          lmer.df = "kenward-roger") %>% # alternative: lmer.df = "satterthwaite"
  pluck("emmeans") %>%
